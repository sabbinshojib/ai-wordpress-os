...
> **2026-09-07:** items 1–8 below (Transaction Engine through Rollback system) have a real, tested implementation including durable encrypted persistence, an execution lease, durable replay protection, and a typed Planner boundary — not exposed to any AI-facing surface, no per-operation crash-recovery journal, no real WordPress/MySQL execution (see `docs/ARCHITECTURE.md` §13 for the exact split). Item 9 (Jobs/background execution) and items 10–20 have not started. See the status note at the top of this document.
