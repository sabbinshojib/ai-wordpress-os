...
> **2026-09-07:** items 1–8 below (Transaction Engine through Rollback system) have a real, tested implementation including durable encrypted persistence, an execution lease, durable replay protection, a typed Planner boundary, and a per-operation crash-recovery journal (classification-only recovery — fails closed to manual review, no automatic continuation) — not exposed to any AI-facing surface, real WordPress/MySQL execution is CI-CONFIGURED-NOT-RUN (see `docs/ARCHITECTURE.md` §13 for the exact split). Item 9 (Jobs/background execution) and items 10–20 have not started. See the status note at the top of this document.
