...
final class Validator {
	...
	private function checkEnum( mixed $value, array $schema, string $path ): array {
		...
		foreach ( $schema['enum'] as $allowed ) {
			// Loose equality on purpose: JSON enums are scalars, and the
			// explicit gettype() equality next to it makes the pair strict.
			/* phpcs:ignore Universal.Operators.StrictComparisons.LooseEqual -- type equality is asserted separately; loose compare is deliberate */
	...
	private function checkString( mixed $value, array $schema, string $path ): array {
		...
		if ( isset( $schema['pattern'] ) && is_string( $schema['pattern'] ) ) {
			...
			if ( false === $result ) {
				// Invalid regex: fail closed as "does not match" rather
				// than silently accepting on a broken pattern.
				$result = 0;
			}
			if ( 0 === $result ) {
