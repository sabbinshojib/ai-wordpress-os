...
final class Sanitize {
	...
	private const SECRET_PATTERNS = array(
		...
		array( 'pattern' => '/(app[_ -]?pass(word)?[\'" :=]+)[A-Za-z0-9 ]{16,}/i', 'replacement' => '$1[REDACTED]' ),
		// Generic key/token assignments in text or JSON.
		array( 'pattern' => '/((?:api[_ -]?key|secret|token|password|passwd|client[_ -]?secret|auth[_ -]?key|access[_ -]?key)[\'" :=]+)[^\'"\\s,;}]{8,}/i', 'replacement' => '$1[REDACTED]' ),
		// sk-style provider keys.
		array( 'pattern' => '/\bsk-[A-Za-z0-9_-]{16,}\b/', 'replacement' => '[REDACTED]' ),
		// Bearer tokens.
		array( 'pattern' => '/\bBearer\s+[A-Za-z0-9._~+\/-]{16,}/i', 'replacement' => 'Bearer [REDACTED]' ),
		// WP salts/constants values when dumped.
		array( 'pattern' => '/((?:AUTH_KEY|SECURE_AUTH_KEY|LOGGED_IN_KEY|NONCE_KEY|AUTH_SALT|SECURE_AUTH_SALT|LOGGED_IN_SALT|NONCE_SALT)[\'" ,:=]+)[\'"][^\'"]{8,}[\'"]/', 'replacement' => '$1\'[REDACTED]\'' ),
		// DB password define.
		array( 'pattern' => '/(DB_PASSWORD[\'" ,:=]+)[\'"][^\'"]*[\'"]/', 'replacement' => '$1\'[REDACTED]\'' ),
	);
	 ...
	 */
	public static function positiveInt( mixed $value, int $default, int $max = PHP_INT_MAX ): int {
	 ...
	 */
	public static function clamp( mixed $value, float $min, float $max, float $default ): float {
		if ( ! is_numeric( $value ) ) {
			$value = $default;
		}
		$num = (float) $value;
