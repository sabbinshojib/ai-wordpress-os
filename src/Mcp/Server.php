...
final class Server {
        ...
        private function toolsCall( JsonRpcRequest $request, WP_User $user, string $client ): JsonRpcResponse {
                ...
                if ( array() === $content ) {
                        $content[] = array(
                                'type' => 'text',
                                'text' => wp_json_encode( $result->data(), JSON_UNESCAPED_SLASHES ) ?: '{}',
                        );
                }
                        ...
                        array(
                                'contents' => array(
                                        array(
                                                'uri'      => $uri,
                                                'mimeType' => 'application/json',
                                                'text'     => wp_json_encode( $payload, JSON_UNESCAPED_SLASHES ) ?: '{}',
                                        ),
                                ),
                        )
                );
        ...
        private function promptsList( JsonRpcRequest $request ): JsonRpcResponse {
                $prompts = array(
                        array(
                                ...
                                'arguments'   => array( array( 'name' => 'focus', 'description' => 'Optional focus area, e.g. performance', 'required' => false ) ),
                        ),
                        array(
                                'name'        => 'redesign-plan',
                                'description' => 'Produce an implementation plan for a homepage refresh using the site map and theme inspection tools.',
                                'arguments'   => array( array( 'name' => 'goal', 'description' => 'Design goal, e.g. premium SaaS look', 'required' => true ) ),
                        ),
                );
