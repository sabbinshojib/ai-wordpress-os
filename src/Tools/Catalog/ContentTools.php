...
final class ContentTools implements CatalogProviderInterface {
					...
					'executeCallback'    => static function ( array $args, $user ) use ( $type ): AbilityResult {
						...
						$result = AbilityResult::success(
							array(
								...
								'link'   => self::permalinkOrNull( $post_id ),
								'slug'   => $post?->post_name ?? '',
							)
						);
					...
					'executeCallback'    => static function ( array $args, $user ) use ( $type ): AbilityResult {
						...
						$result = AbilityResult::success(
							array(
								...
								'link'   => self::permalinkOrNull( $updated ),
	...
	private static function writeSchema( string $type, bool $is_create ): array {
		$properties = array(
			'title'   => array(
				'type'        => 'string',
				'maxLength'   => 500,
				'description' => 'Title (plain text).',
			),
			'content' => array(
				'type'        => 'string',
				'maxLength'   => 500000,
				'description' => 'Content (HTML or block markup). Treated as untrusted site content by the client.',
			),
			'excerpt' => array(
				'type'      => 'string',
				'maxLength' => 5000,
		...
		if ( 'page' === $type ) {
			$properties['template'] = array(
				'type'        => 'string',
				'maxLength'   => 200,
				'description' => 'Page template file name, e.g. page-full-width.php',
			);
		}
	...
	private static function summary( \WP_Post $post ): array {
		return array(
			...
			'link'     => self::permalinkOrNull( $post ),
	...
	/**
	 * Permalink with an explicit null fallback for non-published posts
	 * (get_permalink() returns false for drafts without a URL yet).
	 *
	 * @param int|\WP_Post $post
	 */
	private static function permalinkOrNull( int|\WP_Post $post ): ?string {
