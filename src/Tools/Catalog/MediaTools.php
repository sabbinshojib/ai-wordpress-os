...
final class MediaTools implements CatalogProviderInterface {
						...
						'inputSchema'        => array(
							...
							'properties'           => array(
								'mime_type' => array(
									'type'        => 'string',
									'maxLength'   => 100,
									'description' => 'Filter by mime type, e.g. image/jpeg. Use image for all images.',
								),
								'search'    => array(
									'type'      => 'string',
									'maxLength' => 200,
					...
					array(
						...
						'inputSchema'        => array(
							...
							'properties'           => array(
								'filename' => array(
									'type'        => 'string',
									'maxLength'   => 200,
									'description' => 'File name including extension, e.g. hero.jpg',
								),
								'content'  => array(
									'type'        => 'string',
									'description' => 'Base64-encoded file content (no data: prefix).',
								),
								'title'    => array(
									'type'      => 'string',
									'maxLength' => 300,
								...
								'alt'      => array(
									'type'        => 'string',
									'maxLength'   => 500,
									'description' => 'Alt text for images (recommended).',
								),
							),
						...
						'executeCallback'    => static function ( array $args, $user ): AbilityResult {
							...
							if ( '' === $filename || '.' === $filename ) {
									...
									// phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions.obfuscation_base64_decode -- decoding client-supplied file bytes for upload, not code obfuscation.
	...
	private static function summary( \WP_Post $attachment ): array {
			...
			return array(
				...
				'url'        => self::urlOrNull( $attachment->ID ),
	...
	private static function detail( \WP_Post $attachment ): array {
		...
		$payload['link']        = self::linkOrNull( $attachment );
		return $payload;
	}

	/**
	 * Attachment URL with an explicit null fallback when the attachment
	 * has no public URL yet.
	 */
	private static function urlOrNull( int $attachment_id ): ?string {
		$url = wp_get_attachment_url( $attachment_id );
		return $url ? $url : null;
	}

	/**
	 * Attachment permalink with an explicit null fallback.
	 */
	private static function linkOrNull( \WP_Post $attachment ): ?string {
		$link = get_permalink( $attachment );
