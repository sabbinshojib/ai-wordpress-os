...
final class SystemTools implements CatalogProviderInterface {
	...
	private const SAFE_OPTIONS = array(
		'blogname', 'blogdescription', 'blog_public', 'siteurl', 'home',
		'posts_per_page', 'date_format', 'time_format', 'timezone_string',
		'start_of_week', 'permalink_structure', 'category_base', 'tag_base',
		'use_smilies', 'default_comment_status', 'comment_moderation',
		'thumbnail_size_w', 'thumbnail_size_h', 'medium_size_w', 'medium_size_h',
		'large_size_w', 'large_size_h', 'default_pingback_flag',
	);
		...
		$abilities->register( Ability::make(
			array(
				...
				'executeCallback' => static function ( array $args, $user ): AbilityResult {
					...
					return AbilityResult::success( array( 'events' => $events, 'total' => $count ) );
				},
			)
		) );
			...
			array(
				...
				'executeCallback'    => static function ( array $args, $user ): AbilityResult {
					...
					$result = AbilityResult::success(
						array( 'object_cache_flushed' => true, 'ai_os_transients_cleared' => true )
					);
