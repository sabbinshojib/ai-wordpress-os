	...
	public static function register( AbilityRegistry $abilities, ToolRegistry $tools ): void {
		$abilities->register( Ability::make(
			array(
				...
				'executeCallback' => static function ( array $args, $user ): AbilityResult {
					...
					return AbilityResult::success( array( 'items' => $items, 'locations' => $locations ) );
				},
			)
		) );
		...
		$abilities->register( Ability::make(
			array(
				...
				'inputSchema' => array(
					'type'       => 'object',
					'properties' => array( 'id' => array( 'type' => 'integer', 'minimum' => 1, 'description' => 'Menu term id (see menus.list).' ) ),
					'required'   => array( 'id' ),
